...
extract-then-confirm: provided sources and interview answers do the talking, every extracted artifact is
...
- `resources/context/STAMP` and `resources/context/{process,messaging,assets,bindings}.md` — bundled-resource reads at Procedure step 0, performed by `managed-context-apply`; an absent `STAMP` is the unmanaged path, and an absent `bindings.md` body is the unmanaged-for-writes path. Neither is capability-mediated: no `runtime/tool-bindings.md` row, and nothing for `connect-tools` to report.
...
- Logical capabilities `render.context_preview` and `review.collect` (Procedure step 3) — the section-loop preview and verdict surfaces; view shapes, item grammar, and routing live in `runtime/tool-bindings.md`; surface resolution per `resources/tool-bindings.md`.
...
- `resources/connect-tools.md` — the readiness-gate step invoked at Procedure step 5, composed per its gate entry; its handed-back report is the step's input.
...
2. **Invoke** `managed-context-apply` (`resources/managed-context-apply.md`) in full — it is the sole owner of detect → compare → note → apply whole → stamp → report replaced edits → report stage-rename fallout → state the asset-link assumption; this step restates none of its clauses.
...
3. Evaluate each artifact against its validity predicate — valid-present or not. A predicate reads an absent *directory* identically to an absent *file* — the artifact is simply not valid-present; no error is surfaced, and step 1 stays a pure read:
  ...
  - `company/bindings.md` — the central write policy. **Optional-if-absent, invalid-if-malformed**, and never a mode input: it is not one of the five results below, because no rep authors it. Absent is valid and means this deployment states no central write rule. Present ⇒ valid or invalid whole, never partially honoured. Valid means: every `##` heading is one of `## CRM Authority`, `## CRM Objects`, `## CRM Fields`, `## Never Write`, `## Write Modes`, `## Value Rules`, `## Tasks`; each carries exactly one fenced `yaml` block that parses; and `## CRM Authority` is present. The other six blocks are optional, and an absent block means "no central rule for this" — that concern keeps its unmanaged behaviour. Block shapes, each a mapping:
    - `## CRM Authority` — `provider`, `org_id`, `environment` (`production` or `sandbox`) and `domain`, all non-empty scalars. The one required block: it names which system and which tenant a write is allowed to reach.
    - `## CRM Objects` — logical target (`account`, `opportunity`, `note`) → that CRM's own object/module/table name. A target with no row is unaddressable, never defaulted to a same-named object.
    - `## CRM Fields` — object → logical field → that CRM's API field name. Exhaustive and closed: a logical field with no row is not written by this deployment. Logical field names come from `work-account` Step 6.5's source list, and `note` is an object here like any other, so a logical field may target it. One reserved key: an optional `qualification:` sub-block per object, whose keys are the verbatim field text of `company/process.md`'s `## Qualification` declaration and whose values are API field names — the same join-key discipline `## Value Rules` states for stage tokens, fanning each declared field into its own column instead of one blob. Every other value is a scalar API name; a write mode is never one of them.
    - `## Never Write` — object → a list of API field names. Outranks `## CRM Fields` wherever the two could disagree, so a name on both lists is a contradiction, not a permission — `runtime/lint.sh` `[write-policy]` refuses it at commit time.
    - `## Write Modes` — object → API field name → `replace` or `append`, and any other value invalidates the file. Keyed by API name like `## Never Write`, not by logical field like `## CRM Fields`, because a mode is a property of the destination field rather than of the value headed for it. An absent block, and a field with no row in a present one, both mean `replace`: that is the unmanaged behaviour this block's absence keeps, said out loud because "no central rule" would otherwise be read as "ask" or "skip" and would change a deployment already shipped. `append` declares that the field's existing content is never discarded, so an `append` a consumer cannot perform that way is dropped with its reason stated — never downgraded to a `replace`, on any degraded path.
    - `## Value Rules` — a `stage` map from `company/process.md`'s own enum tokens to that CRM's picklist values, plus optional `date_format` and `text_max_len` scalars. Its left-hand keys are the same tokens that file defines; a key that is not one of them is the divergence `managed-context-apply` step 7 reports.
    - `## Tasks` — `provider`, a `destination` mapping (`container_id`, and `container_name` for narration only), `title_convention`, and a `status_map` from Unbound's own status tokens to that provider's labels.
    No block carries a credential of any kind. Prose between the blocks is free-form, and is what a consumer quotes when it narrates a rule to a rep.
...
**1.7 — State initialization (`state`) (slot).** Contract: one lightweight checkpoint — two rep-answered facts, one read-back, one whole write. Deliberately not a section-loop pass: no pool read, no provenance overlay, no gap markers, and no verdict — both values are rep answers by construction, so the answers are the approval.
...
3. **Read both facts back** — one line in the rep-facing register `render.setup_progress` pins for `title` and `why`: the confirmed zone, the resolved starting date, that the starting point is saved now, and that asking to change it later reopens this checkpoint (the explicit re-ask step 7.3 names). A statement, not a gate — no item is collected on it.
4. **Write whole, once** — write `state/run-state.yaml` under the step-3 write-whole rule (complete, single write, creating the file's parent directory if absent, predicate and `runtime/lint.sh` clean at the moment of writing): exactly the three required keys — `last_run` (full ISO 8601 with offset), `timezone` (IANA zone name), `events: []` — with `scan_window` omitted as the documented default.
  - Where step 0's apply has already written `context_stamp` and `context_applied_at`, this write carries both forward unchanged; where it has not, neither key is written. Their terms are `managed-context-apply`'s `## Writes` to state.
...
5. **Progress** — re-render via `render.setup_progress` with `state` marked done.
...
- **`bindings → company/bindings.md`.** Not a section and never a loop pass — it has one writer, `managed-context-apply` at step 0, from the pack's own `context/bindings.md`. This loop never drafts it, previews it, or takes a verdict on it: a deployment's write policy is central by construction, so there is nothing here for a rep to author or accept, and its absence is never a gap. Per-block extraction is the step-1 predicate's own shapes, read in this order and no other: `## CRM Authority` first, because a wrong tenant sends a rep's pipeline into another organisation; then `## CRM Objects`, since a field map means nothing without the object it hangs on; then `## CRM Fields` and `## Never Write` as one pass, because the second is only meaningful against the first; then `## Write Modes`, keyed off the API names that pass has just settled; then `## Value Rules`, which cannot be extracted before this session's `process` pass has settled the enum its `stage` keys must come from; then `## Tasks`. The pack-side authoring skill extracts against that predicate and restates none of it — a schema quoted from memory is a defect, exactly as it already is for the other three managed files.
...
2. **Receive, never re-derive** — the report is consumed as handed back: this step performs no capability derivation and no tool-inventory enumeration (see Reads — capability derivation belongs to `connect-tools`). Any binding `connect-tools` makes during the gate runs under its own sole write authority and asks this step for nothing; this step collects no `review.collect` item and writes nothing. On a first run in a working tree that has no `runtime/tool-bindings.md`, that same gate call may seed the file from its bundled reference — `connect-tools`' Procedure step 0, its write under the same sole authority; this skill neither performs it, gates it, nor reports it as its own. The same call also installs the **runtime half** into the tree when it is absent (step 0.5), on identical terms: setup is how a rep's tree acquires the deterministic pre-slate path, and the install's narration and any stamp drift belong to the handed-back report, never to this step.
...
2. **Continue** — from the entry point, run the remaining flow exactly as first-run would:
  ...
  - The section loop (step 3) visits only sections not yet valid-present, per its own rule; the state checkpoint (step 1.7) runs only if `state/run-state.yaml` is invalid or absent; the gate (step 5) always runs before close.
...
4. **Constructional guarantee** — a session killed at any instant leaves each artifact either untouched or complete-and-valid: no partially written file is possible, and no repair path is needed.
  - The basis is the write protocol alone: every write is whole-file-after-decision, in dependency order — the step-3.6 write-whole rule and the step-1.7.4 state write — and the write of file N is never gated on file N+1.
  - A mid-section interruption loses only that section's un-accepted draft, never an accepted artifact.

**7 — Refresh & audit (slot).** Contract: the diff-and-propose form of steps 2–3 — audit the existing artifacts against a fresh source pool, present the per-section proposed changes, and apply only what the user accepts. Every consumed rule is a pointer to a carried step, and every write is accept-gated (see Writes).
...
3. **Diff per section** — for `process`, `messaging`, `assets`, `voice` in that fixed order, extract candidates from the pool under the step-3 extract rules and per-section specifics, provenance labels included.
  ...
  - The `state` section is deliberately not audited — it reopens only on the user's explicit ask, which routes to the step-1.7 checkpoint and runs it unchanged — both facts re-asked, the file rewritten whole, no verdict on that path either.
...
2. **Scope** — map the instruction onto `(section, block)` targets, where a block is the same unit step 7.3 declares: a stage in `process`; a contract part, or a block within one, in `messaging`; a table row in `assets`; a tone rule, or a block within one, in `voice`. The resulting per-section target sets are what the step-7.5 loop walks.
  ...
  - This slot's section enum is `process | messaging | assets | voice`. An instruction naming `state` is outside it and routes to the step-1.7 checkpoint on the user's explicit re-ask — the step-7.3 rule that `state` reopens only on that ask, consumed unchanged.
...
6. **Mixed utterance** — handled per step 1's tie-break: this slot runs first, then the flow falls through to step 7.2 re-intake for the document half.
...
- `state/run-state.yaml` — setup-time initialization only, at step 1.7, on the rep's two answers and no verdict; advancing `last_run` afterward stays with the run loop.
  - Step 0's apply writes `context_stamp` and `context_applied_at` alone, and never `last_run`, `timezone`, `scan_window`, `events`, or `work`. Step 1.7's write runs after it and carries those two keys forward where they are already present.
- Never written by a managed apply: `rep/voice.md`, `accounts/`, `projects/`, and any partial subset of the three managed files.
- Never written by this skill: the **content** of `accounts/` and `projects/` — step 1.5 creates those two empty *directories*, but every file under them (`context.md`, drafts, task lists) stays bootstrap-context/run-loop owned — `state/feedback-log.jsonl` (no line, ever — that log stays the run loop's prioritization instrument), `runtime/tool-bindings.md` and the runtime half — `runtime/helpers/`, `resources/templates/`, `runtime/RUNTIME-STAMP` — (sole writer for all of them: `connect-tools`, including the first-run seed and install its step-5 gate invocation may perform), and anything external.
- With steps 0–8 in this file, steps 1 and 2 remain pure reads; the first write in the flow is step 0's managed apply on the managed path or step 1.5's empty-directory creation otherwise, and the first *content* write is step 1.7's seed. Steps 5 and 6 add no write of their own. Steps 7 and 8 write only through the accept-gated whole-file writes above, and never `state/run-state.yaml` — its section reopens only on the explicit ask step 7.3 names.
...
- Never write without an accept verdict on the covering `context_section` item; silence is never a verdict. Exactly two writes stand outside it: the step-0 managed apply, and the step-1.7 seed, whose two rep answers are the approval.
